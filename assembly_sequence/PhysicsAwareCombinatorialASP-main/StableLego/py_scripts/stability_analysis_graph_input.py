...
def stability_score(lego_structure, lego_library, world_dim_ovr=None, config_fname="./config.json"):
    ############### Setup ###############
    ...
    if(world_dim_ovr is not None):
        world_dim = world_dim_ovr
    alpha = config["Alpha"]
    beta = config["Beta"]
    world_grid = construct_world_grid(lego_structure, world_dim, brick_library)  # z starts from 1.
...
if __name__ == '__main__':
    lego_structure = load_json("./lego_structures/stair_19/task_graph.json")
    lego_lib = load_json("./lego_library.json")
    analysis_score, num_vars, num_constr, total_t, solve_t = stability_score(lego_structure, lego_lib, config_fname="./StableLego/config.json")
    print(solve_t, total_t)