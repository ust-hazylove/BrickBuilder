...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Stable Fast 3D and optionally brickize the output mesh.")
    parser.add_argument("--repo_root", required=True, help="Path to the stable-fast-3d repo root.")
    ...
    parser.add_argument("--device", default=None, help="Optional device flag passed to Stable Fast 3D.")
    parser.add_argument("--texture_resolution", type=int, default=None, help="Optional texture resolution override.")
    parser.add_argument("--foreground_ratio", type=float, default=None, help="Optional foreground ratio override.")
    args = parser.parse_args()

    repo_root = Path(args.repo_root).resolve()
    input_image = Path(args.input_image).resolve()
    case_id = args.case_id or slugify(input_image.stem)
    case_dir = build_case_dir(Path(args.output_root), "sf3d", case_id)
    ...
    if args.foreground_ratio is not None:
        command.extend(["--foreground-ratio", str(args.foreground_ratio)])

    summary = init_summary("sf3d", case_id, "image", str(input_image), repo_root, case_dir)
    ...
    mesh_candidates = [
        raw_dir / "0" / "mesh.glb",
        raw_dir / "mesh.glb",
        raw_dir / input_image.stem / "mesh.glb",
    ]
    ...
    print(f"Saved Stable Fast 3D case to: {case_dir}")
